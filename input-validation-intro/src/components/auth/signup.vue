    ...
    <div class="signup-form">
      <form @submit.prevent="onSubmit">
        ...
        <div class="input" :class="{ invalid: $v.age.$error }">
          ...
          <input type="number" @blur="$v.age.$touch()" id="age" v-model.number="age" />
...
export default {
  ...
  validations: {
    email: {
      ...
      //custom validator that checks in the backend if the value is
      // unique(value) {
      ...
      //   return axios
      //     .get(`/vue-data.json?orderBy="email"&equalTo="${value.toString()}"`)
      //     .then(res => {
    ...
    confirmPassword: {
      // sameAs: sameAs("password") // using the property name associated to our v-model
      sameAs: sameAs(vm => {
        return vm.password; // function syntax that allow to compare multiple fields
      })
    },
    terms: {
      sameAs: sameAs(vm => vm.terms === true || vm.country !== "germany")
    },
    hobbyInputs: {
      required,
      minLen: minLength(2),
